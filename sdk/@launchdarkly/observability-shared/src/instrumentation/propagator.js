...
export class CustomTraceContextPropagator extends W3CTraceContextPropagator {
    ...
    inject(context, carrier, setter) {
        ...
        if (typeof url === 'string') {
            const shouldRecord = shouldRecordRequest(url, this.internalEndpoints, this.tracingOrigins, this.urlBlocklist);
            if (!shouldRecord) {
                span.setAttribute(RECORD_ATTRIBUTE, false); // used later to avoid additional processing
            }
            const shouldTrace = shouldNetworkRequestBeTraced(url, this.tracingOrigins ?? [], this.urlBlocklist);
            if (!shouldTrace) {
